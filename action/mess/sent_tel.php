...
$sql="select count(`id`) from `send` where `status`='1'&&`login_id`='{$_SESSION['id']}'&&`tel`='$type'";
$count = $db->get_one($sql, MYSQL_NUM);
$page = new page(array('total'=>$count[0],'perpage'=>8));
$tmp = $page->offset();
$page->show();

$sql="select * from `send` where `status`='1'&&`login_id`='{$_SESSION['id']}'&&`tel`='$type'$tmp";
